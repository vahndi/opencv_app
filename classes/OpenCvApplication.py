...
class OpenCvApplication(PygameApplication):
    ...
    def __init__(self):
        ...
        hsvMapButtonArgs = (('apply hsv map', 'Apply to Image', False, self.toggleApplyHSVmap, None, None),
                            ('replace values', 'Replace Values', False, self.toggleReplaceHSVvalues, None, None))
        ...
        hsvMapSaveButtons = PygamePushButtonSet(self._appDisplay, [10,350],
                                                (('save additive', 'Save Include', self._saveIncludeHSVregion),
                                                 ...
                                                 ('write image', 'Write Image', self._saveImage)),
    ...
    def toggleReplaceHSVvalues(self):
        self._replaceHSVvalues = not self._replaceHSVvalues
...
#            self._photo = OpenCvImage(self._photo.image().transpose(1, 0, 2))
    ...
    def _saveImage(self):
        ...
        if self._photo is not None:
            ...
            processedImage = self.processImage(capImage)
            processedImage.saveAs('tempOpenCv.jpg')
...
#                    print 'replacing HS'
                    image = image.replaceHueSaturationRange(self._findHSVrange[0], self._findHSVrange[1],
                                                            self._replaceHSVrange[0], self._replaceHSVrange[1])
        # Face-tracking                
        if self._trackFaces:
        ...
        if self._drawHoughCircles:
            image.drawCircles(image.getHoughCircles())
    ...
    def drawScreen(self):
        ...
        if capImage:
            image = self.processImage(capImage)
